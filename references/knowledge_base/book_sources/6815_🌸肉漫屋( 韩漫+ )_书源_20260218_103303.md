...
## 基本信息
- **书源名称**: 🌸肉漫屋( 韩漫+ )
- **书源地址**: https://roum26.xyz
- **书源分组**: 漫画,🌸漫画
- **书源类型**: 图片 (2)
- **创建时间**: 2026-02-18 10:33:03
- **来源**: /yuedu/shuyuan/content/id/6815.html
- **发布时间**: 2025/12/30
- **下载次数**: 未知
...
### 1. 选择器规则分析
- **搜索列表**: //div[@class='grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3 sm:gap-4 my-6']/div
- **章节列表**: //div[@class='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 px-2 py-4']/a[starts-with(@href,'/books')]
- **正文内容**: //script/text()@js:
var urlReg = /\\"imageUrl\\":\\"(.+?)\\"/g;
var pureUrlList = [];
var match;
